...
const DaftarProduk = () => {
  ...
  const filtered = products
    .filter((p) => (cat === 'SEMUA' || p.category === cat)
      && (channel === 'SEMUA' || (p.channel || 'KOM') === channel)
    ...
    <div className="space-y-6">
      ...
      <div className="card-surface p-6">
        <div className="flex flex-wrap gap-3 mb-5">
          ...
          <select value={channel} onChange={(e) => setChannel(e.target.value)} className="bg-[#0b0f17] border border-[#242f3d] rounded-lg px-3 py-2.5 text-sm outline-none focus:border-[#2563eb] min-w-[130px]"><option value="SEMUA">Semua Saluran</option><option value="PSO">PSO</option><option value="KOM">KOM</option></select>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm tbl">
            <thead><tr className="text-left border-b border-[#1a222e]">{['Nama Produk', 'SKU', 'Saluran', 'Kategori', 'Stok Baik', 'Konversi Kemasan', 'Stok Rusak', 'Harga Modal', 'Nilai Total', 'Supplier / Lokasi', 'Aksi'].map((h) => <th key={h} className="py-2.5 pr-4 font-semibold whitespace-nowrap">{h}</th>)}</tr></thead>
            <tbody>
              {filtered.length === 0 ? <tr><td colSpan={11} className="py-8 text-center text-[#6b7688]">Tidak ada produk.</td></tr> : filtered.map((p) => (
