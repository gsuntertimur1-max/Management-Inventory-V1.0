...
const Pengeluaran = () => {
  ...
  const returnItemsFor = (load, mode, sourceDocumentNo = '') => (load.items || []).filter((item) => mode !== 'SO_RETUR' || (item.documentNo || load.ref) === sourceDocumentNo).map((item) => {
    const remaining = mode === 'SO_RETUR' ? salesReturnRemaining(load, item, sourceDocumentNo) : remainingQty(load, item);
  ...
  const openLinkedDocument = (load, mode) => {
    const sourceDocumentNo = mode === 'SO_RETUR' ? (load.documents || [load.ref])[0] || '' : '';
    setDocumentModal({ load, mode, documentNo: '', note: '', sourceDocumentNo, items: isReturnMode(mode) ? returnItemsFor(load, mode, sourceDocumentNo) : (load.items || []).map((item) => ({ productId: item.productId, name: item.name, unit: item.unit, qty: remainingQty(load, item), remaining: remainingQty(load, item) })) });
  };
