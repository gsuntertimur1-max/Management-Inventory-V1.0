...
const statusLabel = (status) => ({
  ...
  ADA_RETUR: 'Ada Retur', SO_TERBIT_SEBAGIAN: 'SO Terbit Sebagian', SELESAI: 'Selesai', DIBATALKAN: 'Dibatalkan',
}[status] || status);
...
const BazarNDExternal = () => {
  ...
  const [actionMeta, setActionMeta] = useState({ date: today(), vehicleNo: '', soNo: '', note: '' });

  const load = async () => setDocuments((await api.get('/bazar/external-nds')).data);
  ...
  const openAction = (type, document) => {
    const rows = {};
    (document.summaryItems || []).forEach((row) => { rows[row.productId] = { goodQty: '', damagedQty: '', qty: '', stackCode: 'BZR/A01' }; });
    setActionRows(rows); setAction({ type, document });
    setActionMeta({ date: today(), vehicleNo: '', soNo: '', note: '' });
  };
  ...
  const submitAction = async () => {
    ...
    if (type === 'receive') {
      ...
      payload = {
        ...
        items: document.summaryItems.map((row) => ({ productId: row.productId, goodQty: Number(actionRows[row.productId]?.goodQty || 0), damagedQty: Number(actionRows[row.productId]?.damagedQty || 0), stackCode: actionRows[row.productId]?.stackCode || 'BZR/A01' })),
      };
    } else if (type === 'return') {
      path = 'returns';
      payload = { returnDate: actionMeta.date, note: actionMeta.note, items: document.summaryItems.filter((row) => Number(actionRows[row.productId]?.qty || 0) > 0).map((row) => ({ productId: row.productId, qty: Number(actionRows[row.productId].qty) })) };
    } else if (type === 'realize') {
      path = 'realizations';
      payload = { realizationDate: actionMeta.date, activityType: document.activityType, referenceNo: actionMeta.soNo, note: actionMeta.note, items: document.summaryItems.filter((row) => Number(actionRows[row.productId]?.qty || 0) > 0).map((row) => ({ productId: row.productId, qty: Number(actionRows[row.productId].qty) })) };
    } else {
      path = 'so-documents';
  ...
  return <div className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        {documents.map((doc) => <div key={doc.id} className="border border-[#243044] rounded-xl p-4">
          ...
          <div className="flex flex-wrap gap-2 mt-3">
            {doc.status !== 'SELESAI' && <button onClick={() => openAction('receive', doc)} className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg border border-[#2563eb]/50 text-[#93c5fd] text-xs"><ArrowDownToLine size={13}/> Terima Fisik</button>}
            {(doc.summaryItems || []).some((row) => row.unsettledQty > 0) && <button onClick={() => openAction('return', doc)} className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg border border-[#f59e0b]/50 text-[#fbbf24] text-xs"><RotateCcw size={13}/> Retur ke Gudang Asal</button>}
            {(doc.summaryItems || []).some((row) => row.physicalBalance > 0) && <button onClick={() => openAction('realize', doc)} className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg border border-[#a855f7]/50 text-[#d8b4fe] text-xs"><BadgeCheck size={13}/> Catat Realisasi</button>}
            {(doc.summaryItems || []).some((row) => row.eligibleSoQty > 0) && <button onClick={() => openAction('so', doc)} className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg border border-[#22c55e]/50 text-[#86efac] text-xs"><FileCheck2 size={13}/> Catat SO Terbit</button>}
          </div>
        </div>)}
      </div>
    </section>
    ...
    {action && <div className="fixed inset-0 z-[90] bg-black/75 flex items-center justify-center p-4"><div className="card-surface w-full max-w-3xl p-6 max-h-[90vh] overflow-y-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 mb-3"><input type="date" className={inputCls} value={actionMeta.date} onChange={(e) => setActionMeta({ ...actionMeta, date: e.target.value })}/>{action.type === 'receive' && <input className={inputCls} placeholder="No. kendaraan" value={actionMeta.vehicleNo} onChange={(e) => setActionMeta({ ...actionMeta, vehicleNo: e.target.value })}/>} {action.type === 'so' && <input className={inputCls} placeholder="Nomor SO" value={actionMeta.soNo} onChange={(e) => setActionMeta({ ...actionMeta, soNo: e.target.value })}/>} {action.type === 'realize' && <input className={inputCls} placeholder="Referensi Bazar/Paket" value={actionMeta.soNo} onChange={(e) => setActionMeta({ ...actionMeta, soNo: e.target.value })}/>}</div>
      <div className="space-y-3">{action.document.summaryItems.map((row) => <div key={row.productId} className="border border-[#243044] rounded-xl p-3"><div className="text-sm font-semibold mb-2">{row.name} · {action.type === 'receive' ? `sisa ND ${row.remainingToReceive}` : action.type === 'so' ? `siap SO ${row.eligibleSoQty}` : `saldo fisik ${row.physicalBalance}`} {row.unit}</div>{action.type === 'receive' ? <div className="grid grid-cols-1 sm:grid-cols-3 gap-2"><input type="number" min="0" className={inputCls} placeholder="Diterima baik" value={actionRows[row.productId]?.goodQty || ''} onChange={(e) => setActionRows((p) => ({ ...p, [row.productId]: { ...p[row.productId], goodQty: e.target.value } }))}/><input type="number" min="0" className={inputCls} placeholder="Diterima rusak" value={actionRows[row.productId]?.damagedQty || ''} onChange={(e) => setActionRows((p) => ({ ...p, [row.productId]: { ...p[row.productId], damagedQty: e.target.value } }))}/><input className={inputCls} placeholder="Lokasi, mis. BZR/A01" value={actionRows[row.productId]?.stackCode || ''} onChange={(e) => setActionRows((p) => ({ ...p, [row.productId]: { ...p[row.productId], stackCode: e.target.value } }))}/></div> : <input type="number" min="0" max={action.type === 'so' ? row.eligibleSoQty : row.physicalBalance} className={inputCls} placeholder={action.type === 'return' ? 'Jumlah kembali ke gudang asal' : action.type === 'realize' ? 'Jumlah terjual/disalurkan' : 'Kuantum dalam SO'} value={actionRows[row.productId]?.qty || ''} onChange={(e) => setActionRows((p) => ({ ...p, [row.productId]: { ...p[row.productId], qty: e.target.value } }))}/>}</div>)}</div>
