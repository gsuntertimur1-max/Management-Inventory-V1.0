...
const Pengeluaran = () => {
  ...
  const startAndPrint = async (load) => {
    ...
    try {
      ...
      await printApiFile(`/export/bon-muat-v2/${updated.id}.pdf`);
  ...
  const finalizeLoading = async (load, payload = {}) => {
    ...
    try {
      ...
      const sj = result?.suratJalan;
      if (sj?.id) {
        try {
          await printApiFile(`/export/surat-jalan/${sj.id}.pdf`);
          toast.success(`Pemuatan selesai · Surat Jalan ${sj.no || ''} diterbitkan · dialog cetak dibuka`);
        } catch (printError) {
          toast.warning(`Pemuatan selesai dan Surat Jalan ${sj.no || ''} sudah terbit, tetapi dialog cetak gagal dibuka. Gunakan tombol Cetak Surat Jalan.`);
        }
      } else {
        toast.success(`Pemuatan selesai · Surat Jalan ${sj?.no || ''} diterbitkan`);
      }
