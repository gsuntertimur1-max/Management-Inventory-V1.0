...
const CatatStok = () => {
  ...
  const submit = async () => {
    ...
    if (type === 'KELUAR' && documentType === 'MEMO' && consignmentDestination && !consignmentZone) {
            ...
            {rows.map((row, index) => {
              ...
              return (
                <div key={index} className={`grid gap-2 items-end p-3 rounded-lg border border-[#1a222e] bg-[#0b0f17] ${type === 'MASUK' ? 'grid-cols-1 md:grid-cols-[minmax(190px,1fr)_105px_145px_175px_52px]' : 'grid-cols-1 md:grid-cols-[minmax(170px,1fr)_100px_135px_150px_140px_52px]'}`}>
                  ...
                  {type === 'KELUAR' && <><div><label className="text-[10px] text-[#6b7688] mb-1 block">Dokumen</label><select value={row.documentNo || documentRefs[0] || ''} onChange={(e) => setRow(index, { documentNo: e.target.value })} className="w-full bg-[#0b0f17] border border-[#242f3d] rounded-lg px-2 py-2.5 text-xs"><option value="">Pilih...</option>{documentRefs.filter(Boolean).map((doc) => <option key={doc}>{doc}</option>)}</select></div><div><label className="text-[10px] text-[#6b7688] mb-1 block">Tumpukan asal</label><select value={row.stackCode || ''} onChange={(e) => setRow(index, { stackCode: e.target.value })} className="w-full bg-[#0b0f17] border border-[#242f3d] rounded-lg px-2 py-2.5 text-xs"><option value="">Otomatis / belum dipilih</option>{STACKS.map((code) => <option key={code}>{code}</option>)}</select></div></>}
