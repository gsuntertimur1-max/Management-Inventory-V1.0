  ...
  return <div className="space-y-5" data-testid="stack-map-page">
    ...
    <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_390px] gap-5">
      ...
      <aside className="card-surface p-5 h-fit xl:sticky xl:top-24">
        <div className="flex justify-between"><div><div className="label-mono text-[10px]">Tumpukan dipilih</div><h2 className="font-display text-2xl font-bold mt-1">{selected}</h2></div><div className="flex gap-2"><button title="Cetak kartu tumpukan PDF" onClick={() => printApiFile(`/export/stack-card.pdf?stackCode=${encodeURIComponent(selected)}`).catch((e) => toast.error(apiError(e)))} className="h-fit p-2 rounded-lg border border-[#242f3d] text-[#60a5fa]"><Printer size={17} /></button>{canWrite && <button onClick={() => setModal({ mode: 'add', data: blank(selected) })} className="btn-primary h-fit px-3 py-2 rounded-lg text-sm flex gap-2"><PackagePlus size={16} />Tambah</button>}</div></div>
