...
const CatatStok = ({ panel = '' }) => {
  ...
  const setSoTakeMode = (index, mode) => {
    ...
    if (mode === 'ALL') {
      ...
      setRow(index, { soTakeMode: 'ALL', documentQty: qty });
      return;
    }
    setRow(index, { soTakeMode: 'PARTIAL' });
  };
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      Promise.all(refs.map(async (doc) => {
        ...
        if (cancelled) return;
        const balances = Object.fromEntries(entries.filter(([, data]) => data));
        setSoBalances(balances);
        // SO yang sudah pernah didaftarkan membawa Penerima/Tujuan master-nya.
        // Isi otomatis hanya kolom yang masih kosong agar input operator tidak ditimpa.
        setDocumentParties((prev) => documentRefs.map((doc, index) => (
          prev[index]
          || balances[String(doc || '').trim().toUpperCase()]?.party
          || ''
        )));
      });
    }, 350);
      ...
      <div className={`grid grid-cols-1 lg:grid-cols-3 gap-6 ${activePanel ? 'hidden' : ''}`}>
        <div className="card-surface p-6 lg:col-span-2">
                  ...
                  <div>
                    ...
                    <SearchableProductSelect
                      ...
                      onChange={(productId) => {
                        ...
                        setTransactionInput(index, { productId, inputMode: 'QTY', inputValue: 1, stackCode: '', documentQty: '', soTakeMode: 'PARTIAL', channel: chosenProduct?.channel || 'KOM', fefoExceptionReason: '' });
                      }}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div><label className="text-sm font-medium mb-1.5 block">{type === 'MASUK' ? 'Supplier Pengirim' : documentType === 'SO' ? 'Penerima / Tujuan' : 'Penerima / Tujuan'}</label>{type === 'MASUK' ? <select value={party} disabled={Boolean(selectedPO)} onChange={(e) => setParty(e.target.value)} className="w-full bg-[#0b0f17] border border-[#242f3d] rounded-lg px-3 py-2.5 text-sm"><option value="">Pilih supplier...</option>{suppliers.map((supplier) => <option key={supplier.id}>{supplier.name}</option>)}</select> : documentType === 'SO' ? <div className="rounded-lg border border-[#1f3657] bg-[#0d1728] px-3 py-2.5 text-xs text-[#93c5fd]">Diisi di samping masing-masing nomor SO di atas.</div> : <input value={party} onChange={(e) => setParty(e.target.value)} placeholder="Nama penerima / tujuan" className="w-full bg-[#0b0f17] border border-[#242f3d] rounded-lg px-3 py-2.5 text-sm" />}</div>
