...
from backend.operational_guards import guarded_complete_outbound
...
async def guarded_complete_outbound_with_fefo(load_id: str, request: Request, user: dict = Depends(require_write)):
    result = await guarded_complete_outbound(load_id, request, user)
