...
const Pengeluaran = () => {
  ...
  const openLinkedDocument = (load, mode) => setDocumentModal({ load, mode, documentNo: '', note: '', items: (load.items || []).map((item) => isReturnMode(mode) ? { productId: item.productId, name: item.name, unit: item.unit, goodQty: 0, damagedQty: 0, stackCode: item.location || '', remaining: remainingQty(load, item) } : { productId: item.productId, name: item.name, unit: item.unit, qty: remainingQty(load, item), remaining: remainingQty(load, item) }) });
                ...
                return (
                  <tr key={load.id} className="tbl-row border-b border-[#131a24] align-top">
                    ...
                    <td className="py-3 pr-4"><div className="flex flex-wrap gap-2 min-w-[270px]">
                      ...
                      {load.status === 'Selesai' && <><button onClick={() => downloadBon(load)} className="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-2 rounded-lg border border-[#242f3d] hover:bg-[#141a24]"><Printer size={13} /> Download Bon</button><button onClick={() => downloadSuratJalan(sj)} className="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-2 rounded-lg border border-[#22c55e] text-[#4ade80] hover:bg-[#22c55e]/10"><Printer size={13} /> Download Surat Jalan</button>{load.document_type === 'CT' && <button onClick={() => openLinkedDocument(load, 'CR')} className="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-2 rounded-lg border border-[#f59e0b] text-[#fbbf24]"><RotateCcw size={13} /> Catat CR</button>}{load.document_type === 'MEMO' && <button onClick={() => openLinkedDocument(load, 'RETUR')} className="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-2 rounded-lg border border-[#f59e0b] text-[#fbbf24]"><RotateCcw size={13} /> Catat Retur</button>}{['CT', 'MEMO'].includes(load.document_type) && <button onClick={() => openLinkedDocument(load, 'SO')} className="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-2 rounded-lg border border-[#2563eb] text-[#60a5fa]"><Link2 size={13} /> Tautkan SO</button>}</>}
                    </div></td>
                  </tr>
                );
              })}
